...
router = APIRouter(prefix="/classifier", tags=["Climate Classifier"])
...
async def train_initial_model(request: InitialTrainingRequest):
    """
    Train initial model with staged training data
...
async def retrain_model(request: RetrainRequest):
    """
    Retrain model with additional labeled data
    ...
    try:
        ...
        return {
            "success": True,
            "message": f"Model retrained successfully: {request.batch_name}",
            "model_name": result['model_name'],
            "training_samples": result['training_samples'],
            "benchmarks": result['benchmarks'],
            "batch_info": result['batch_info'],
...
async def pseudo_label_data(request: PseudoLabelRequest):
    """
    Pseudo-label all staged unlabeled data
    
    Process:
    1. Loads all unlabeled CSVs from staged_unlabeled directory
    2. Predicts labels using current model
...
async def predict_single(request: PredictionRequest):
    """
    Make a prediction on a single text using the current model
    """
    try:
        service = get_classifier_service()
...
async def get_system_info():
    ...
    try:
        ...
        return {
            ...
            "workflow": {
                "step_1": "Upload training CSV files (text, label columns)",
